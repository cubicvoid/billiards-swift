...
func mergeConstraint<k: Field & Comparable>(
    ...
    withList constraints: [Vec3<k>]) -> SphericalPolygon<k>? {
...
func mergeConstraints<k: Field & Comparable>(
    ...
    withList constraintList: [Vec3<k>]) -> SphericalPolygon<k>? {
...
public enum SphericalPolygon<
  ...
  public static func fromConstraints(
      _ constraints: [Vec3<k>]) -> SphericalPolygon {
...
private func chooseBasisForPlane<k: Field>(
    normal: Vec3<k>) -> [Vec3<k>] {
  let one = k.one
  let zero = k.zero
  let canonical: [Vec3<k>] = [
    Vec3(x: zero, y: one, z: zero),
