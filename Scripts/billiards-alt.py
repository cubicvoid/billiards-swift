...
class PathGeometryRing:
  def __init__(self, apex, maxAngles):
    ...
    r0 = z0.times(z0)
    r1 = z1.times(z1)
    self.rotation = [z0, z1] #[r0, r1]
    #self.norms = [z0.squaredLength(), z1.squaredLength()]
    self.denominators = [z0.conjugate(), z1.conjugate()]
    self.edges = [
        Complex(1, 0), Complex(apex.x - 1, apex.y), Complex(-apex.x, -apex.y)]

    denPowers = [[Complex(1, 0)], [Complex(1, 0)]]
    for i in range(2):
      denArray = denPowers[i]
      for _ in range(maxAngles[i]):
        denArray.append(denArray[-1].times(self.denominators[i]))
    self.denPowers = denPowers
    ...
    for i in range(2):
      ...
      for _ in range(maxAngles[i]):
        rotArray.append(rotArray[-1].times(self.rotation[i]))
    self.rotationPowers = rotationPowers
    ...
    for i in range(2):
      ...
      for n in range(maxAngles[i] + 1):
        cacheArray.append(
            rotationPowers[i][n] * denPowers[i][maxAngles[i] - n])
    self.cachedPowers = cachedPowers
...
class PathStudy:
  ...
  def boundarySpineCount(self):
    count = 0
    for pathEdge in self.spineEdges:
      if pathEdge.fromVert.pos.side == pathEdge.toVert.pos.side:
        count += 1
    return count
  ...
  def fieldConstraintFunctions(self, apex):
    pg = PathGeometryField(apex)
    spineOffsets = [
...
def BoundarySpineCountForEdgePath(edgePath):
  fans = FansForEdgePath(edgePath)
  count = 0
  for fan in fans:
    if fan.startsOnBaseEdge:
      count += 1
  return count
...
def CountsForEdgePath(edgePath):
  ...
  for fan in fans:
    ...
    counts.append(fan.orientation * baseLength)
...
cf(3, 4).factor() # the extremal pair for the lower boundary
...
class PathStats:
  def __init__(self, counts, edgePath):
    self.edgePath = edgePath
    self.path = PathCountForm(counts)
    self.pathStr = str(self.path)
    self.boundarySpineCount = BoundarySpineCountForEdgePath(edgePath)
    #self.pathStudy = PathStudy(edgePath)
    #self.boundarySpineCount = self.pathStudy.boundarySpineCount()

    """ancestorPath = self.path.minimalAncestor()
    if self.path != ancestorPath:
      self.ancestor = PathStats.statsForCountForm(ancestorPath)
    else:
      self.ancestor = None"""
  ...
  @staticmethod
  def statsForEdgePath(edgePath):
    counts = CountsForEdgePath(edgePath)
    path = PathCountForm(counts)
    pathStr = str(path)

    if pathStr not in allPathStats:
      allPathStats[pathStr] = PathStats(counts, edgePath)
    return allPathStats[pathStr]

  @staticmethod
  def statsForCountForm(countForm):
    pathStr = str(countForm)

    if pathStr not in allPathStats:
      allPathStats[pathStr] = PathStats(countForm.counts)
    return allPathStats[pathStr]
...
def PathStatsForTextFile(filename):
  with open(filename) as f:
    for line in f:
      edgePath = line.strip()
      if len(edgePath) == 0: continue
      stats = PathStats.statsForEdgePath(edgePath)
      stats.dataPoints += 1
      #if stats.ancestor:
      #  stats.ancestor.descendantDataPoints += 1

  spineCounts = {}
  for pathStats in allPathStats.values():
    oldCount = 0
    key = str(pathStats.boundarySpineCount)
    if key in spineCounts:
      oldCount = spineCounts[key]
    spineCounts[key] = oldCount + pathStats.dataPoints
  ...
  """
  print("Spine counts:")
  for count in spineCounts:
    print(str(count) + ": " + str(spineCounts[count]))
  """
  minimalPaths = [
